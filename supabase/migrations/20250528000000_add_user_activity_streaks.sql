...
DROP TABLE IF EXISTS public.user_activity_streaks;
...
CREATE INDEX idx_user_activity_streaks_user_id ON public.user_activity_streaks(user_id);
CREATE INDEX idx_user_activity_streaks_last_active_date ON public.user_activity_streaks(last_active_date);
...
-- Create policy to allow users to insert their own activity streaks
CREATE POLICY "Users can insert their own activity streaks" 
    ON public.user_activity_streaks
    FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- Allow users to use the RLS policies
GRANT SELECT, INSERT, UPDATE ON public.user_activity_streaks TO authenticated; 