...
export function MobileNavigation() {
  ...
  return (
    <>
      ...
      >
        <nav className="flex items-center justify-around py-0.5 w-full">
          {navItems.map((item) => (
            <Link 
              key={item.path}
              to={item.path} 
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button 
                variant="ghost" 
                size="icon"
                className="touch-manipulation h-10 w-10 sm:h-12 sm:w-12 rounded-full flex items-center justify-center bg-white/20 shadow-sm border border-gray-200/30 active:scale-95 transition-transform"
              >
      ...
      <AnimatePresence>
        <motion.div 
          ...
          transition={{ type: "spring", stiffness: 300, damping: 30 }}
          className="fixed bottom-20 right-4 flex flex-col gap-2 z-50 lg:hidden"
        >
          <QuickActionButtons />
          ...
          <AnimatePresence>
            {showToTop && (
              <motion.button
                ...
                className="touch-manipulation h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg flex items-center justify-center border border-gray-200/30 active:scale-95 transition-transform"
              >
